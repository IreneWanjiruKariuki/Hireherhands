...
document.addEventListener("DOMContentLoaded", () => {
  ...
  switch (currentFilter) {
    case "requests":
      endpoint = "/jobs/worker-requests";
      break;
    case "ongoing":
      endpoint = "/worker/jobs"; // we’ll filter status client-side
      break;
    case "completed":
      endpoint = "/worker/jobs"; // we’ll filter status client-side
      break;
    default:
      endpoint = "/worker/jobs"; // for "all"
      break;
  }
  ...
  try {
    ...
    // For "ongoing" or "completed", filter status client-side
...
async function loadJobs() {
  ...
  try {
    ...
    if (currentFilter === "ongoing") jobs = jobs.filter(job => job.status === "in_progress");
    else if (currentFilter === "completed") jobs = jobs.filter(job => job.status === "completed");
...
function showJobDetails(jobJson) {
  ...
  content.innerHTML = `
    <div class="job-detail-grid">
      ...
      <div class="detail-item"><label>Budget:</label><span>KSh ${job.budget}</span></div>
      ...
      <div class="detail-item"><label>Status:</label><span>${job.status}</span></div>
    </div>`;
